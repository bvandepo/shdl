...
public class SHDLModule {
	...
	public String getVHDLSignalDeclaration(SHDLSignal signal, String ioStatus, String header, boolean isLast) {
		...
		if (dir == -1)
			// -1 : scalar
			sb.append(header + getVHDLName(signal, arity, false) + " : " + ioStatus + " std_logic");
		else if (dir == 0) {
			if (signal.isPartOfBus())
	...
	public String getVHDLIntSignalDeclaration(SHDLSignal signal, String header) {
		...
		if (dir == -1)
			// -1 : scalar
			return(header + "signal " + getVHDLName(signal, arity, false) + " : std_logic ;");
		else if (dir == 0) {
	...
	public String getVHDLName(SHDLSignal signal, int constArity, boolean substitute) {
		int arity = signal.getArity();
	...
	public String getVHDLText(SHDLSignalOccurence so, int arity) {
		String sigText = getVHDLName(so.getSignal(), arity, true);
	...
	public String getVHDLText(SHDLCombinatorialSetting cs) {
		...
		if (cs.getOE() == null) {
			if (signal.getArity() == 1) {
				return ("\t" + getVHDLName(signal, arity, true) + " <= " + getVHDLText(cs.getEquation(), arity) + " ;");
			} else if (cs.getEquation().containsScalars()) {
				StringBuffer sb = new StringBuffer();
				sb.append("\t" + getVHDLName(signal, arity, true) + " <= ");
					...
					for (int j = 0; j < sigOccs.size(); j++) {
						...
						for (int k = 0; k < arity; k++) {
							...
							if (sigOcc.isInverted())
								sb.append("(not " + getVHDLName(sigOcc.getSignal(), arity, true) + ")");
							else
								sb.append(getVHDLName(sigOcc.getSignal(), arity, true));
						}
						sb.append(")");
					}
					sb.append(")");
			...
			return ("\t" + getVHDLName(signal, arity, true) + " <= " + getVHDLText(cs.getEquation(), arity) + " when " + getVHDLName(cs.getOE().getSignal(), arity, true) + " = '" + val + "' else " + nthChar(arity, 'Z') + " ;");
		} else {
			StringBuffer sb = new StringBuffer();
	...
	public String getVHDLText(SHDLSequentialSetting ss) {
		...
		StringBuffer sb = new StringBuffer();
		sb.append("\tprocess (" + getVHDLName(ss.getCLK().getSignalOccurence().getSignal(), arity, true));
		if (ss.getRST() != null) sb.append(", " + getVHDLName(ss.getRST().getSignalOccurence().getSignal(), arity, true));
		if (ss.getSET() != null) sb.append(", " + getVHDLName(ss.getSET().getSignalOccurence().getSignal(), arity, true));
		sb.append(") begin" + newline);
		boolean first = true;
		if (ss.getRST() != null) {
			int rstVal = ss.getRST().getSignalOccurence().isInverted() ? 0 : 1;
			sb.append("\t\tif " + getVHDLName(ss.getRST().getSignalOccurence().getSignal(), arity, true) + " = '" + rstVal + "' then" + newline);
			sb.append("\t\t\t" + getVHDLName(ss.getSignal(), arity, true) + " <= " + nthChar(arity, '0') + " ;" + newline);
			first = false;
		}
		if (ss.getSET() != null) {
			int setVal = ss.getSET().getSignalOccurence().isInverted() ? 0 : 1;
			sb.append("\t\t" + (first ? "if " : "elsif ") + getVHDLName(ss.getSET().getSignalOccurence().getSignal(), arity, true) + " = '" + setVal + "' then" + newline);
			sb.append("\t\t\t" + getVHDLName(ss.getSignal(), arity, true) + " <= " + nthChar(arity, '1') + " ;" + newline);
		}
		...
		if (ss.getENA() != null) {
			...
			enas = getVHDLName(ss.getENA().getSignalOccurence().getSignal(), arity, true) + " = '" + enaVal + "' and ";
		}
		sb.append("\t\t" + iff + enas + getVHDLName(ss.getCLK().getSignalOccurence().getSignal(), arity, true) + "'event and " + getVHDLName(ss.getCLK().getSignalOccurence().getSignal(), arity, true) + " = '" + clkVal + "' " + "then" + newline);
		sb.append("\t\t\t" + getVHDLName(ss.getSignal(), arity, true) + " <= " + getVHDLText(ss.getEvolution(), arity) + " ;" + newline);
