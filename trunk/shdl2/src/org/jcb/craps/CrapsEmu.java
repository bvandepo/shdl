	...
	class LoadObjActionListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			try {
				...
				JOptionPane.showMessageDialog(frame, "Le code de ce module entre en conflit avec celui des autres, à partir de l'adresse : " + ex.getAddr());
			} catch(Exception ex) {
				ex.printStackTrace();
			}
		}
...
//System.out.println("globalSymbolTable=" + globalSymbolTable);
	...
	public String disassemble(long addr, long instr) {
		...
		switch ((int) op) {
			...
			case 1:	// reti
				return "reti";
			case 2:	// arithmétique & logique
			case 3:	// accès mémoire
	...
	private void updateViews() {
		try {
			// on vide les caches car après un run ou un step,
			// les caches n'ont pas été prévenus des changements
			regCache.clear();
			memCache.clear();
	...
	class RunActionListener implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			if (runButton.getActionCommand().equals("run")) {
				// cliquer sur run, c'est demander un 'step' puis mettre run à 1
